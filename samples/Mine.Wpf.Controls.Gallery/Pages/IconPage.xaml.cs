...
public partial class IconPage : Mine.Wpf.Controls.Controls.Page
{
    ...
    // 图标数据：字段名 + 显示名称
    private record IconItem(string FieldName, string DisplayName, bool Fill);

    private static readonly (string FieldName, string DisplayName)[] AllIcons = typeof(MaterialIcons)
        .GetFields(BindingFlags.Public | BindingFlags.Static)
        .Where(f => f.IsLiteral && f.FieldType == typeof(string))
        .Select(f =>
        {
            var unicode = (string)f.GetRawConstantValue()!;
            // 过滤掉补充私用区字符（代理对，WPF 无法正确渲染）
            return unicode.Length == 1 ? (FieldName: f.Name, DisplayName: ToSnakeCase(f.Name)) : default;
        })
        .Where(t => t != default)
    ...
    private void ApplyFilter(string query)
    {
        var filtered = string.IsNullOrEmpty(query)
            ? AllIcons
            : AllIcons.Where(t => t.DisplayName.Contains(query, StringComparison.OrdinalIgnoreCase));

        // 每个 IconItem 携带字段名和当前 Fill 值
        var rows = filtered
            .Select(t => new IconItem(t.FieldName, t.DisplayName, _currentFill))
            .Chunk(_perRow)
            .ToList();
